...
def scholar():
    try:
        ...
        print(f"Se han cargado {len(data)} registros de publicaciones")
        print(f"Se han cargado {len(authors)} autores")
        ...
        # print(f"ERROR: {e}")
        # traceback.print_exc()
        print("(Scholar) Guardando datos en temporal...")
        save_data_on_temporal(data, latest_index)
        scholar_summary["complete"] = False
    finally:
        print("(Scholar) Guardando resumen...")
        save_summary({"scholar": scholar_summary})
